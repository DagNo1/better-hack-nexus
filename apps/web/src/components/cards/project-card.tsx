...
import { Clock, Edit2, FileText, Folder, Trash2 } from "lucide-react";
...
export function ProjectCard({ project, onEdit, onDelete }: ProjectCardProps) {
  ...
  const totalItems = (project.files?.length || 0) + (project.folders?.length || 0);
    ...
    <Card className="group hover:shadow-lg transition-all duration-200 hover:scale-[1.02] cursor-pointer border border-border hover:border-primary/20 bg-card">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2 text-sm text-muted-foreground">
              <div className="flex items-center gap-1 w-full flex-row">
                ...
                <span className="text-xs">Updated {formatDate(project.updatedAt)}</span>
              </div>
              {totalItems > 0 && (
                <>
      ...
      <CardContent className="pt-0">
        <div className="flex items-center justify-between text-sm">
          ...
          {(project.files && project.files.length > 0) || (project.folders && project.folders.length > 0) ? (
